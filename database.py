...
class VNBusinessImporter:
    ...
    def process_areas(self, df: pd.DataFrame) -> Dict[str, int]:
        ...
        try:
            # Process districts
            districts = df['Quận / Huyện'].unique()
            for district in districts:
                if pd.notna(district):
                    ...
                    ward_mask = df['Quận / Huyện'] == district
                    wards = df.loc[ward_mask, 'Phường/xã'].unique()
    ...
    def process_business_types(self, df: pd.DataFrame) -> Dict[str, int]:
        ...
        try:
            business_types = df['Loại hình DN'].unique()
            for btype in business_types:
                if pd.notna(btype):
                    cur.execute(
                        "INSERT INTO business_type (descr) VALUES (%s) ON CONFLICT DO NOTHING RETURNING id",
                        (btype,)
                    )
    ...
    def process_activities(self, df: pd.DataFrame) -> Dict[str, int]:
        ...
        try:
            ...
            # Add main activities
            main_activities = list(map(lambda y: list(map(lambda x: x.removesuffix(',').split(':'), re.findall(r"[\d]{4,}:[\D]*", str(y)))), df['Ngành nghề KD chính'].dropna()))
            ...
            other_activities = list(map(lambda y: list(map(lambda x: x.removesuffix(',').split(':'), re.findall(r"[\d]{4,}:[\D]*", str(y)))), df['Ngành nghề KD'].dropna()))
            ...
            for code, activity in all_activities.items():
                cur.execute(
                    "INSERT INTO activities (code, descr) VALUES (%s, %s) ON CONFLICT DO NOTHING RETURNING code",
                    (code, activity)
                )
    ...
    # def process_legal_reps(self, df: pd.DataFrame) -> Dict[str, int]:
    ...
    #     try:
    #         legal_reps = df['Người đại diện theo pháp luật'].unique()
    #         for rep in legal_reps:
    #             if pd.notna(rep):
    #                 cur.execute(
    #                     "INSERT INTO legal_rep (name) VALUES (%s) RETURNING id",
    #                     (rep,)
    #                 )
    ...
    def import_data(self):
        ...
        try:
            ...
            # rep_map = self.process_legal_reps(df)
            ...
            try:
                for _, row in df.iterrows():
                    # Insert business
                    area_key = f"{row['Quận / Huyện']}|{row['Phường/xã']}"
                    ...
                    """, (
                        ...
                        row['Mã số doanh nghiệp'],
                        row['Địa chỉ trụ sở chính'],
                        area_id,
                        self.process_phone_numbers(row['Điện thoại']),
                        row['Email'],
                        row['Vốn điều lệ'],
                        type_map.get(row['Loại hình DN']),
                        row.get('Loại DN') == 'TN'
                    ))
                    ...
                    if pd.notna(row['Ngành nghề KD chính']):
                        main_act_code, _ = list(map(lambda x: x.removesuffix(',').split(':'), re.findall(r"[\d]{4,}:[\D]*", row['Ngành nghề KD chính'])))
                        if main_act_code:
                            cur.execute("""
                    ...
                    if pd.notna(row['Ngành nghề KD']):
                        other_acts =list(map(lambda x: x.removesuffix(',').split(':'), re.findall(r"[\d]{4,}:[\D]*", row['Ngành nghề KD'])))
                    ...
                    for shareholder_list in ['DS Thành viên góp vốn', 'DS cổ đông']:
                    ...
                    if pd.notna(row.get('Người đại diện theo pháp luật')):
                        rep = row['Người đại diện theo pháp luật']
                        cur.execute(
...
def main():
    ...
    # Excel file path
    excel_file = 'dsdn_1997_2024_cleaned.xlsx'
